...
if ( ! class_exists( 'Mime' ) ) {
	 ...
	 * @version 3.2.8
	 * @link http://wordpress.org/plugins/rrze-downloads/ MimeTypes Link Icons WordPress plugin
	 * @link https://github.com/eagerterrier/rrze-downloads GitHub development of MimeTypes Link Icons WordPress plugin
	 *
	...
	class Mime {
		 ...
		 */
		const SETTINGS_OPTION = 'rrze-downloads';

		/**
		 * @const	string	Name of options variable containing the filesize cached values
		 */
		const CACHE_OPTION = 'rrze-downloads_filesize_cache';
		...
		// 	add_action( 'rrze-downloads_plugin_activate', array( $this, 'upgrade_options' ) );
		// 
		// 
		...
		// 
		// 	// error_log('filter_option_defaults : $this->defaults=' . implode('|', $this->defaults));
		// 
...
// 
// 
// error_log('self::SETTINGS_OPTION = ' . self::SETTINGS_OPTION);
// 
// 
// 			$this->settings = $value;
// 
...
// 		}
		...
		// 	load_plugin_textdomain( 'rrze-downloads', false, self::$name . '/languages' );
		// 
		...
		// 	self::$name = apply_filters( 'rrze-downloads_plugin_name', self::$name );
		// }
		...
		// 		'general'	=> __( 'General Settings', 'rrze-downloads' ),
		// 		'images'	=> __( 'Image Settings', 'rrze-downloads' ),
		// 		'advanced'	=> __( 'Advanced Settings', 'rrze-downloads' ),
		// 	);
		// 
		// 	$this->byte_suffixes = array(
		// 		_x( 'b', 'Abbreviation of "byte"', 'rrze-downloads' ),
		// 		__( 'kB', 'rrze-downloads' ),
		// 		__( 'MB', 'rrze-downloads' ),
		// 		__( 'GB', 'rrze-downloads' ),
		// 		__( 'TB', 'rrze-downloads' ),
		// 		__( 'PB', 'rrze-downloads' ),
		// 		__( 'EB', 'rrze-downloads' ),
		// 		__( 'ZB', 'rrze-downloads' ),
		// 		__( 'YB', 'rrze-downloads' ),
		// 	);
		// }
		// 
		...
		// 		if ( false === $this->settings['use_async'] || true === $this->settings['filesize'] ) {
		...
		// 		__( 'TEST Downloads', 'rrze-downloads' ), /* page title */
		// 		__( 'TEST Downloads', 'rrze-downloads' ), /* menu title */
		...
		// 		$links[] = '<a href="' . esc_url( $this->plugin_options_url() ) . '" alt="' . esc_attr__( 'RRZE Downloads Settings', 'rrze-downloads' ) . '">' . esc_html__( 'Settings', 'rrze-downloads' ) . '</a>';
		// 	}
		// 	return $links;
		// }
		// 
		...
		// 		add_query_arg(
		// 			'cssvars',
		// 			base64_encode( 'mtli_height=' . $this->settings['icondimensions'] . '&mtli_icontype=' . $this->settings['icontype'] . '&mtli_iconalign=' . $this->settings['iconalign'] ),
		...
		// 
		// 
		// 	if ( ( true === $this->settings['use_classnames'] && ( is_array( $this->settings['classnames'] ) && array() !== $this->settings['classnames'] ) ) || ( $this->settings['replacement'] == 'asynchronous' && ( is_array( $this->active_mimetypes ) && array() !== $this->active_mimetypes ) ) ) {
		// 		wp_enqueue_script(
		// 			self::$name, // id
		...
		// 	$strings = array(
		// 		'hidethings'			=> ( ( true === $this->settings['use_classnames'] && ( is_array( $this->settings['classnames'] ) && array() !== $this->settings['classnames'] ) ) ? true : false ),
		// 		'use_async'			=> ( ( 'asynchronous' == $this->settings['replacement'] && ( is_array( $this->active_mimetypes ) && array() !== $this->active_mimetypes ) ) ? true : false ),
		// 		'use_async_debug'	=> ( ( true === $this->settings['asynchronous_debug'] && ( is_array( $this->active_mimetypes ) && array() !== $this->active_mimetypes ) ) ? true : false ),
		// 	);
		// 
		// 	/* Add jQuery class selector string if hidden classes are used */
		// 	if ( 'asynchronous' == $this->settings['replacement'] && ( is_array( $this->settings['classnames'] ) && array() !== $this->settings['classnames'] ) ) {
		// 		$strings['classnames'] = '';
		// 		foreach ( $this->settings['classnames'] as $classname ) {
		// 			$strings['classnames'] .= '.' . $classname . ',';
		// 		}
		// 		$strings['classnames'] = substr( $strings['classnames'], 0, -1 );
		// 	}
		// 
		// 	/* Add array of active mimetypes if in async mode*/
		// 	if ( 'asynchronous' == $this->settings['replacement'] && ( is_array( $this->active_mimetypes ) && array() !== $this->active_mimetypes ) ) {
		// 		$strings['icons_mimetypes'] = $this->active_mimetypes;
		// 	}
		// 
		// 	return $strings;
		// }
		...
		// 	$strings = array(
		// 		'togglebox'     => '<div class="check-images"><span class="check-all">' . esc_html__( 'Check All', 'rrze-downloads' ) . '</span>|<span class="uncheck-all">' . esc_html__( 'Uncheck All', 'rrze-downloads' ) . '</span></div>',
		// 	);
		// 	return $strings;
		// }
		// 
		...
		// 				'title'   => __( 'Advanced Settings', 'rrze-downloads' ),
		// 				'callback' => array( $this, 'get_helptext' ),
		// 			)
		// 		);
		// 		$screen->add_help_tab(
		...
		// 				'title'   => __( 'Extras', 'rrze-downloads' ),
		// 				'callback' => array( $this, 'get_helptext' ),
		// 			)
		// 		);
		// 
		...
		// 						sprintf( __( 'The <em><a href="%s">MimeTypes Link Icons</a></em> plugin will automatically add an icon next to links of the activated file types. If you like, you can also let the plugin add the file size of the linked file to the page.', 'rrze-downloads' ), 'http://wordpress.org/plugins/rrze-downloads/" target="_blank" class="ext-link' ) . '</p>
		// 						<p>' . esc_html__( 'On this settings page you can specify the icon size, icon type (white matte gif or transparent png), icon alignment. You can also select the file types for which this plugin will be enabled.', 'rrze-downloads' ) . '</p>';
		// 
		// 	$helptext[ self::$name . '-advanced' ] = '
		// 						<p>' . __( 'In the advanced settings, you can enable <em>"exclusion classnames"</em>, enable the display of the <em>file size</em> of a linked file and/or choose to use <em>asynchronous replacement</em>.', 'rrze-downloads' ) . '</p>
		// 						<p>' . __( '<strong>"Exclusion classnames"</strong> works as follows:', 'rrze-downloads' ) . '<br />
		// 						' . esc_html__( 'The plugin will look for the classname in your document and will remove the Mimetypes link icons (and file sizes) from all links wrapped within that class. You can add several classnames, just separate them with a comma.', 'rrze-downloads' ) . '</p>';
		// 
		// 	$helptext[ self::$name . '-extras' ] = '
		// 						<p>' . __( 'There is even some more advanced functionality available: for instance an <em>output filter</em> for the file size output and a way to add the plugin\'s functionality to widgets or other areas of your blog outside of the main content area.', 'rrze-downloads' ) . '</p>
		// 
		// 						<p>' .
		// 						/* Translators: %1$s = <a> tag, %2$s is closing</a> tag. */
		// 						sprintf( esc_html__( 'For more information on these tasty extras, have a look at the %1$sFAQ%2$s', 'rrze-downloads' ), '<a href="http://wordpress.org/plugins/rrze-downloads/faq/" target="_blank" class="ext-link">', '</a>' ) . '</p>';
		// 
		// 
		...
		// 	return '
		// 		   <p><strong>' . esc_html__( 'For more information:', 'rrze-downloads' ) . '</strong></p>
		// 		   <p>
		// 				<a href="http://wordpress.org/plugins/rrze-downloads/" target="_blank">' . esc_html__( 'Official plugin page', 'rrze-downloads' ) . '</a> |
		// 				<a href="http://wordpress.org/plugins/rrze-downloads/faq/" target="_blank">' . esc_html__( 'FAQ', 'rrze-downloads' ) . '</a> |
		// 				<a href="http://wordpress.org/plugins/rrze-downloads/changelog/" target="_blank">' . esc_html__( 'Changelog', 'rrze-downloads' ) . '</a> |
		// 				<a href="http://wordpress.org/support/plugin/rrze-downloads" target="_blank">' . esc_html__( 'Support&nbsp;Forum', 'rrze-downloads' ) . '</a>
		// 			</p>
		// 		   <p><a href="https://github.com/eagerterrier/rrze-downloads" target="_blank">' . esc_html__( 'Github repository', 'rrze-downloads' ) . '</a></p>
		// 		   <p><a href="http://blog.eagerterrier.co.uk/2010/10/holy-cow-ive-gone-and-made-a-mime-type-wordpress-plugin/" target="_blank">' . esc_html__( 'Blog post about this plugin', 'rrze-downloads' ) . '</a></p>
		// 	';
		// }
		...
		// 	do_action( 'rrze-downloads_plugin_activate' );
		// }
 ...
 // 
	...
	// 					if ( ( true === $this->settings['use_hidden_class'] && ( is_array( $this->settings['hidden_classname'] ) && array() !== $this->settings['hidden_classname'] ) ) && ( ! is_null( $classnames ) && '' !== $classnames ) ) {
 ...
 // 
	...
	// 					if ( true === $this->settings['filesize'] ) {
 ...
 // 
	...
	// 					if ( false === $this->settings['use_async'] ) {
 ...
 // 
	...
	// 		if ( true === $this->settings['filesize'] && ( is_array( $this->filesize_styles ) && array() !== $this->filesize_styles ) ) {
 ...
 // 
	...
	// 		if ( true === $this->settings['use_cache'] && ( isset( $this->cache[ $cache_key ] ) && $this->cache[ $cache_key ]['time'] > ( time() - $this->settings['cachetime'] ) ) ) {
 ...
 // 
	...
	// 					trigger_error( sprintf( __( 'MimeTypes Link Icons can\'t resolve the following url, please make sure the file referred to exists. URL: %s', 'rrze-downloads' ), esc_attr( $url ) ), E_USER_NOTICE );
	// 				}
	// 			}
	// 			return $filesize;
	// 		}
 ...
 // 
	// 		// error_log('VALIDATE: ' . implode('|', $this));
 // 
 ...
 // 
	// 		// error_log( 'clean:' . implode('|', $clean ) . '#' ); // clean hat bereits die alten oder default werte und das ist der bug 
 // 
 // 
 ...
 // // error_log(' icondimensions=>ERROR $received=' . implode('|', $received) . ' $key=' . $key . ' $received[ $key ]=' . $received[ $key ] . '$this->sizes=' . implode('|', $this->sizes)); 
 // 
	// 						add_settings_error( self::SETTINGS_OPTION, $key, esc_html__( 'Invalid image size received', 'rrze-downloads' ) . ', ' . esc_html__( 'the setting has not been changed.', 'rrze-downloads' ), 'error' );
	// 					}
	// 					break;
 // 
 // 
	// 				case 'icontype':
	// 					if ( isset( $received[ $key ] ) && in_array( $received[ $key ], $this->icontypes ) ) {
	// 						$clean[ $key ] = $received[ $key ];
	// 					}
	// 					else if ( function_exists( 'add_settings_error' ) ) {
	// 						// Edge case: should never happen
	// // 						// error_log(' icontype=>ERROR' ); 
	// 						add_settings_error( self::SETTINGS_OPTION, $key, esc_html__( 'Invalid image type received', 'rrze-downloads' ) . ', ' . esc_html__( 'the setting has not been changed.', 'rrze-downloads' ), 'error' );
	// 					}
	// 					break;
 // 
 // 
	...
	// 					else if ( function_exists( 'add_settings_error' ) ) {
	// 						// Edge case: should never happen
	// 	// 					// error_log(' iconalign=>ERROR' ); 
	// 						add_settings_error( self::SETTINGS_OPTION, $key, esc_html__( 'Invalid image placement received', 'rrze-downloads' ) . ', ' . esc_html__( 'the setting has not been changed.', 'rrze-downloads' ), 'error' );
	// 					}
	// 					break;
 // 
 // 
	...
	// 						}
	// 						else if ( function_exists( 'add_settings_error' ) ) {
	// 		// 					// error_log(' precision=>ERROR' ); 
	// 							add_settings_error( self::SETTINGS_OPTION, $key, esc_html__( 'Invalid rounding precision received', 'rrze-downloads' ) . ', ' . esc_html__( 'the setting has not been changed.', 'rrze-downloads' ), 'error' );
	// 						}
	// 						unset( $int );
	// 					}
	// 					else {
 ...
 // 
	...
	// 						}
	// 						else if ( function_exists( 'add_settings_error' ) ) {
	// 			// 				// error_log(' 1 cachetime=>ERROR' ); 
	// 							add_settings_error( self::SETTINGS_OPTION, $key, esc_html__( 'Invalid cache time received', 'rrze-downloads' ) . ', ' . esc_html__( 'the setting has not been changed.', 'rrze-downloads' ), 'error' );
	// 						}
	// 						unset( $int );
	// 					}
 ...
 // 
	...
	// 				// 					// error_log(' 1 classnames=>ERROR' ); 
	// 									add_settings_error( self::SETTINGS_OPTION, $key, esc_html__( 'One or more invalid classname(s) received, the values have been cleaned - this may just be the removal of spaces -, please check.', 'rrze-downloads' ), 'updated' );
	// 								}
	// 							}
	// 							else if ( function_exists( 'add_settings_error' ) ) {
	// 								// Edge case: should never happen
	// 				// 				// error_log(' 2 classnames=>ERROR' ); 
	// 								add_settings_error( self::SETTINGS_OPTION, $key, esc_html__( 'No valid classname(s) received', 'rrze-downloads' ) . ', ' . esc_html__( 'the setting has not been changed.', 'rrze-downloads' ), 'error' );
	// 							}
	// 						}
	// 						else {
 ...
 // 
	...
	// 		if ( false === $clean['cache'] && $clean['cache'] !== $this->settings['cache'] ) {
 ...
 // 
	...
	// 	}
		...
		// 		wp_die( __( 'You do not have sufficient permissions to access this page.', 'rrze-downloads' ) );
		// 	}
		// 
		// 	echo '
		// <div class="wrap">
		...
		// 
		// 	// echo '<pre>';
		// 	// error_log( 'SETTINGS: '. implode('|'. $this->settings) );
		// 	// echo '</pre>';
		// 
		// 
		// }
		...
		// 
		// 	add_filter( 'mtli_setting_select_box_option_label_icondimensions', array( $this, 'icondimensions_option_label' ) );
		// 
		// 	$this->do_select_box_row( __( 'Image Size', 'rrze-downloads' ), 'icondimensions', $this->sizes );
		// 
		// 	/* @todo maybe change this to radio buttons ? */
		// 	$this->do_select_box_row( __( 'Image Type', 'rrze-downloads' ), 'icontype', $this->icontypes );
		// 
		// 	/* @todo maybe change this to radio buttons ? */
		// 	$this->do_select_box_row( __( 'Display images on left or right', 'rrze-downloads' ), 'iconalign', $this->alignments );
		// 
		// 	echo '
		// 		</table>
		// 	</fieldset>';
		...
		// 					sprintf( __( 'Add images to <strong>%s</strong> uploads/files', 'rrze-downloads' ), $mime_type ) . '</label>
		// 				</th>
		// 				<td style="width:24px;"><img src="' . esc_url( plugins_url( '/images/' . $mime_type . '-icon-24x24.png', __FILE__ ) ) . '" alt="' .
		// 				/* Translators: %s = file type. */
		// 				esc_attr( sprintf( __( '%s icon', 'rrze-downloads' ), $mime_type ) ) . '" /></td>
		// 				<td>
		...
		// 		<legend>' . esc_html__( 'Enable/Disable classnames?', 'rrze-downloads' ) . '</legend>
		// 		<table width="100%" cellspacing="2" cellpadding="5" class="editform form-table">
		// 			<tr>
		// 				<td><label for="use_hidden_class"><input type="checkbox" name="' . esc_attr( self::SETTINGS_OPTION . '[use_hidden_class]' ) . '" id="use_hidden_class" value="on" ' . checked( $this->settings['use_hidden_class'], true, false ) . ' /> ' . __( 'Tick this box to have one or more <em>classname(s)</em> that will disable the mime type links (ie: around an image or caption).', 'rrze-downloads' ) . '</label></td>
		// 			</tr>
		// 			<tr>
		// 				<td><label for="hidden_classname">' . esc_html__( 'You can change the classname(s) by editing the field below. If you want to exclude several classnames, separate them with a comma (,).', 'rrze-downloads' ) . '</label></td>
		// 			</tr>
		// 			<tr>
		...
		// 		<legend>' . esc_html__( 'Show File Size?', 'rrze-downloads' ) . '</legend>
		// 		<table width="100%" cellspacing="2" cellpadding="5" class="editform form-table">
		// 			<tr>
		// 				<td><label for="filesize"><input type="checkbox" name="' . esc_attr( self::SETTINGS_OPTION . '[filesize]' ) . '" id="filesize" value="on" ' . checked( $this->settings['filesize'], true, false ) . ' /> ' . __( 'Display the <em>file size</em> of the attachment/linked file.', 'rrze-downloads' ) . '</label></td>
		// 				<td>
		// 					<label for="precision">' . esc_html__( 'File size rounding precision:', 'rrze-downloads' ) . '
		// 					<input type="text" name="' . esc_attr( self::SETTINGS_OPTION . '[precision]' ) . '" id="precision" value="' . esc_attr( $this->settings['precision'] ) . '" /> ' . esc_html__( 'decimals', 'rrze-downloads' ) . '</label><br />
		// 					<small><em>' . esc_html__( 'sizes less than 1kB will always have 0 decimals', 'rrze-downloads' ) . '</em></small>
		// 				</td>
		// 			</tr>
		// 			<tr>
		// 				<td colspan="2">' . esc_html__( 'Retrieving the file sizes of (external) files can be slow. If the file sizes of the files you link to do not change very often, you may want to cache the results. This will result in faster page loading for most end-users of your website.', 'rrze-downloads' ) . '</td>
		// 			</tr>
		// 			<tr>
		// 				<td><label for="use_cache"><input type="checkbox" name="' . esc_attr( self::SETTINGS_OPTION . '[use_cache]' ) . '" id="use_cache" value="on" ' . checked( $this->settings['use_cache'], true, false ) . ' /> ' . esc_html__( 'Cache retrieved file sizes.', 'rrze-downloads' ) . '</label></td>
		// 				<td>
		// 					<label for="cachetime">' . esc_html__( 'Amount of time to cache retrieved file sizes:', 'rrze-downloads' ) . '
		// 					<input type="text" name="' . esc_attr( self::SETTINGS_OPTION . '[cachetime]' ) . '" id="cachetime" value="' . esc_attr( round( $this->settings['cachetime'] / ( 60 * 60 ), 0 ) ) . '" /> ' . esc_html__( 'hours', 'rrze-downloads' ) . '</label>
		// 				</td>
		// 			</tr>
		// 		</table>
		// 	</fieldset>
		...
		// 		<legend>' . esc_html__( 'Enable Asynchronous Replacement?', 'rrze-downloads' ) . '</legend>
		// 		<table width="100%" cellspacing="2" cellpadding="5" class="editform form-table">
		// 			<tr>
		// 				<td colspan="2">' . esc_html__( 'Some themes or plugins may conflict with this plugin. If you find you are having trouble you can switch on asynchronous replacement which (instead of PHP) uses JavaScript to find your links.', 'rrze-downloads' ) . '</td>
		// 			</tr>
		// 			<tr>
		// 				<td><label for="use_async"><input type="checkbox" name="' . esc_attr( self::SETTINGS_OPTION . '[use_async]' ) . '" id="use_async" value="on" ' . checked( $this->settings['use_async'], true, false ) . ' /> ' . __( 'Tick box to enable <em>asynchronous replacement</em>.', 'rrze-downloads' ) . '</label></td>
		// 				<td><label for="use_async_debug"><input type="checkbox" name="' . esc_attr( self::SETTINGS_OPTION . '[use_async_debug]' ) . '" id="use_async_debug" value="on" ' . checked( $this->settings['use_async_debug'], true, false ) . ' /> ' . __( 'Tick box to enable <em>asynchronous debug mode</em>.', 'rrze-downloads' ) . '</label></td>
		// 			</tr>
		// 		</table>
		// 	</fieldset>';
		// }
		...
		//  * @param	string
		//  */
		// public function icondimensions_option_label( $size ) {
		// 	return ( $size . 'x' . $size );
		// }
		// 
	...
	// }
	// 
	// if ( ! function_exists( 'rrze_downloads_init' ) ) {
	// 	/**
	// 	 * Initialize the class
	// 	 *
	// 	 * @return void
	// 	 */
	// 	function rrze_downloads_init() {
	// 		/* Initialize the static variables */
	// 		RRZE_Downloads::init_statics();
	// 
	// 		$GLOBALS['rrze_downloads'] = new RRZE_Downloads();
	// 	}
	// }
	// 
	// 
	...
	// 			$async = $GLOBALS['mimetypes_link_icons']->settings['use_async'];
	// 			$GLOBALS['mimetypes_link_icons']->settings['use_async'] = false;
	// 
	// 			$content = $GLOBALS['mimetypes_link_icons']->mimetype_to_icon( $content );
	// 			$GLOBALS['mimetypes_link_icons']->settings['use_async'] = $async;
	// 			unset( $async );
	// 		}
	// 
	// 		return $content;
	...
	// 		if ( isset( $GLOBALS['rrze-downloads'] ) && ( ( false === $GLOBALS['rrze-downloads']->settings['use_async'] || true === $GLOBALS['rrze-downloads']->settings['filesize'] ) && false === has_filter( 'the_content', array( $GLOBALS['rrze-downloads'], 'mimetype_to_icon' ) ) ) ) {
	// 			add_filter( 'the_content', array( $GLOBALS['rrze-downloads'], 'mimetype_to_icon' ), 15 );
	// 		}
	// 	}
	// }

	/* Set up the (de-)activation actions */
	// register_activation_hook( __FILE__, array( 'rrze-downloads', 'activate' ) );
